...
local function wrapTab(rawTab)
    ...
    -- Force Left groupbox (không ảnh hưởng _nextIsRight counter)
    function wrapped:AddSectionLeft(name)
        _currentSection = rawTab:AddLeftGroupbox(name or " ")
        return _currentSection
    end

    -- Force Right groupbox (không ảnh hưởng _nextIsRight counter)
    function wrapped:AddSectionRight(name)
        _currentSection = rawTab:AddRightGroupbox(name or " ")
        return _currentSection
    end
...
-- TweenBlock: blocking version dùng cho bay đến NPC (cần chờ đến nơi)
function TweenBlock(p176)
    ...
    pcall(function()
        ...
        local v178 = TweenSpeed
        local v179 = TweenInfo.new(v177 / v178, Enum.EasingStyle.Linear)
        local v180 = game:GetService("TweenService"):Create(player.Character.HumanoidRootPart, v179, {["CFrame"] = p176})
        v180:Play()
        if _G.StopTween then v180:Cancel() return end
        _G.Clip2 = true
        wait(v177 / v178)
        _G.Clip2 = false
    end)
    ...
    while task.wait() do
        if _G.AutoLevel then
            pcall(function()
                ...
                -- Chưa có quest → bay đến NPC nhận quest (TweenBlock vì cần đến nơi)
                ...
                -- Đã có quest đúng → farm (Tween non-blocking như code gốc)
...
-- ===== CÂU CÁ - dùng AddSection để _nextIsRight counter không bị lệch =====
-- Tại đây _nextIsRight=true (từ AddSection "Cày Cấp") nên AddSection sẽ tạo Right đúng
-- và reset _nextIsRight=false để các section sau (Chiến Đấu...) render đúng thứ tự
Tabs.Main:AddSection("Câu Cá")
...
Tabs.Main:AddDropdown("DropdownFishBait", {
    ["Title"] = "Chọn Mồi Câu",
    ...
    ["Callback"] = function(v)
        _G.SelectedBait = v
        pcall(function()
            local req = game.ReplicatedStorage:FindFirstChild("FishReplicated")
            if req then req:FindFirstChild("FishingRequest"):InvokeServer("SelectBait", v) end
        end)
    end
})
Tabs.Main:AddDropdown("DropdownFishRod", {
    ["Title"] = "Chọn Cần Câu",
...
-- ===== FARM LEVEL 2600-2750 - Left groupbox =====
-- AddSectionLeft không đụng _nextIsRight, giờ _nextIsRight=false nên AddSection("Chiến Đấu") sẽ đúng
Tabs.Main:AddSectionLeft("Farm Level 2600-2750")

-- Loop câu cá - chạy sau khi UI đã render xong hoàn toàn
task.spawn(function()
    local l_FishReplicated_BCH = nil
    local l_FishingRequest_BCH = nil
    local l_MaxLaunchDistance_BCH = 50
    local l_GetWaterHeight_BCH = nil
    pcall(function()
        l_FishReplicated_BCH = game.ReplicatedStorage:WaitForChild("FishReplicated", 10)
        if l_FishReplicated_BCH then
            l_FishingRequest_BCH = l_FishReplicated_BCH:WaitForChild("FishingRequest", 10)
            pcall(function()
                l_MaxLaunchDistance_BCH = require(l_FishReplicated_BCH.FishingClient.Config).Rod.MaxLaunchDistance
            end)
            pcall(function()
                l_GetWaterHeight_BCH = require(game.ReplicatedStorage.Util.GetWaterHeightAtLocation)
            end)
        end
    end)
    while task.wait() do
        if _G.AutoFishing and l_FishingRequest_BCH then
            local l_lp = game.Players.LocalPlayer
            local l_char = l_lp.Character
            local l_hrp = l_char and l_char:FindFirstChild("HumanoidRootPart")
            local l_tool = l_char and l_char:FindFirstChildOfClass("Tool")
            if _G.SelectedRod and (not l_tool or l_tool.Name ~= _G.SelectedRod) then
                local l_rod = l_lp.Backpack:FindFirstChild(_G.SelectedRod)
                if l_rod then
                    l_lp.Character.Humanoid:EquipTool(l_rod)
                    l_tool = l_rod
                end
            end
            if l_char and l_hrp and l_tool then
                local l_waterY = 0
                if l_GetWaterHeight_BCH then
                    pcall(function() l_waterY = l_GetWaterHeight_BCH(l_hrp.Position) end)
                end
                local l_hitPos = nil
                pcall(function()
                    local _, l_hit = workspace:FindPartOnRayWithIgnoreList(
                        Ray.new(l_char.Head.Position, l_hrp.CFrame.LookVector * l_MaxLaunchDistance_BCH),
                        {l_char, workspace:FindFirstChild("Characters"), workspace:FindFirstChild("Enemies")}
                    )
                    if l_hit then
                        l_hitPos = Vector3.new(l_hit.X, math.max(l_hit.Y, l_waterY), l_hit.Z)
                    end
                end)
                local l_state = l_tool:GetAttribute("State")
                local l_serverState = l_tool:GetAttribute("ServerState")
                if l_state ~= "ReeledIn" and l_serverState ~= "ReeledIn" or not l_hitPos then
                    if l_serverState == "Biting" then
                        pcall(function()
                            l_FishingRequest_BCH:InvokeServer("Catching", true)
                            task.wait(0.1)
                            l_FishingRequest_BCH:InvokeServer("Catch", 1)
                        end)
                    end
                else
                    if l_hitPos then
                        pcall(function()
                            l_FishingRequest_BCH:InvokeServer("StartCasting")
                            task.wait()
                            l_FishingRequest_BCH:InvokeServer("CastLineAtLocation", l_hitPos, 100, true)
                        end)
                    end
                end
            end
...
Tabs.Main:AddToggle("ToggleAutoFarmLevelNew", {
    ["Title"] = "Auto Farm Level 2600-2750",
    ["Description"] = "Cày Cấp Ở Đảo Submerged (Dưới Nước)",
    ["Default"] = false
}):OnChanged(function(v)
    _G.AutoFarmLevelNew = v
end)
MonNew = MonNew or ""
LevelQuestNew = LevelQuestNew or 1
NameQuestNew = NameQuestNew or ""
NameMonNew = NameMonNew or ""
CFrameQuestNew = CFrameQuestNew or CFrame.new(0,0,0)
CFrameMonNew = CFrameMonNew or CFrame.new(0,0,0)
            ...
            pcall(function()
                ...
                if l_Quest_0.Visible == false then
                    ...
                    if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - CFrameQuestNew.Position).Magnitude > 20 then
                        if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - CFrameQuestNew.Position).Magnitude > 3000 then
                            pcall(function()
                                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", CFrameQuestNew.Position)
                            end)
                        end
                        Tween2(CFrameQuestNew)
                    else
                        ...
                        if v519.Name == MonNew and v519:FindFirstChild("HumanoidRootPart") and v519:FindFirstChild("Humanoid") and v519.Humanoid.Health > 0 then
                            if not string.find(l_Quest_0.Container.QuestTitle.Title.Text, NameMonNew) then
                                ...
                                repeat
                                    ...
                                    EquipTool(SelectWeapon)
                                    AutoHaki()
                                    Tween2(v519.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    v519.HumanoidRootPart.CanCollide = false
                                    v519.Humanoid.WalkSpeed = 0
                                    if v519:FindFirstChild("Head") then v519.Head.CanCollide = false end
...
-- Bảng lưu connection đã hook
local _boneHpConnections = {}
spawn(function()
    while wait(0.2) do
        if _G.OneHitKill then
            pcall(function()
                for _, mob in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if mob:FindFirstChild("Humanoid") and not _boneHpConnections[mob] then
                        -- Hook HealthChanged để clamp về 50% ngay khi server restore
                        local hum = mob.Humanoid
                        _boneHpConnections[mob] = hum.HealthChanged:Connect(function(newHp)
                            if _G.OneHitKill and newHp > hum.MaxHealth * 0.25 then
                                pcall(function()
                ...
                -- Dọn connection của mob đã chết/despawn
                for mob, conn in pairs(_boneHpConnections) do
                    if not mob.Parent then
                        pcall(function() conn:Disconnect() end)
                        _boneHpConnections[mob] = nil
                    end
                end
            end)
        else
            -- Tắt toggle → disconnect tất cả
            for mob, conn in pairs(_boneHpConnections) do
                pcall(function() conn:Disconnect() end)
            end
            _boneHpConnections = {}
        ...
        while wait() do
            if _G.AutoBone then
                pcall(function()
                    ...
                    -- Tìm quái xương và đánh (Tween non-blocking như code gốc)
        ...
        while wait() do
            if _G.AutoBoneNoQuest then
                pcall(function()
                    ...
                    -- Không có mob: đứng chờ spawn, không bay về đâu cả
                end)
            end
        end
    end)
...
-- ===== AUTO SKILL trong Raid =====
Tabs.Raid:AddSection("Auto Skill")
Tabs.Raid:AddToggle("ToggleRaidSkillZ", {
    ["Title"] = "Auto Skill Z",
...
Tabs.Raid:AddToggle("ToggleRaidSkillX", {
    ["Title"] = "Auto Skill X",
...
Tabs.Raid:AddToggle("ToggleRaidSkillC", {
    ["Title"] = "Auto Skill C",
...
-- ===== AUTO KILL PLAYER TRONG TRIAL (V4) =====
Tabs.Raid:AddSection("Đánh Người Chơi")
Tabs.Raid:AddToggle("ToggleKillV4", {
    ["Title"] = "Auto Kill Player Trial V4",
    ["Description"] = "Đánh Người Chơi Trong Trial/Gần Mình",
    ["Default"] = false
}):OnChanged(function(v)
    _G.AutoKillV4 = v
end)
            ...
            pcall(function()
                for _, v in pairs(game.Workspace.Characters:GetChildren()) do
                    if v.Name ~= game.Players.LocalPlayer.Name
                        ...
                        and v.Humanoid.Health > 0 and v.Parent
                        and (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude <= 230
                    then
                        repeat
                            task.wait()
                            AutoHaki()
                            EquipTool(SelectWeapon)
                            Tween2(v.HumanoidRootPart.CFrame * CFrame.new(1, 1, 2))
                            v.HumanoidRootPart.Size = Vector3.new(60, 60, 60)
                            v.HumanoidRootPart.CanCollide = false
                            v.Humanoid.WalkSpeed = 0
                            sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                        until not _G.AutoKillV4 or v.Humanoid.Health <= 0 or not v.Parent
                    end
                end
            end)
        end
...
-- ===============================================================
-- ===========================================================================================
-- PHẦN MỚI: CÁC CHỨC NĂNG LẤY TỪ HDANH HUB
-- ===========================================================================================

-- [Câu Cá và Farm Level 2600-2750 đã được chuyển vào phần Cày Cấp phía trên]

-- ===== SPEED & JUMP SLIDER =====
Tabs.Player:AddSection("Tốc Độ & Nhảy")
getgenv().WalkSpeedValue = getgenv().WalkSpeedValue or 30
getgenv().JumpValue = getgenv().JumpValue or 50
local function _applySpeedJump(char)
    ...
    if hum then
        ...
        hum:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
            if getgenv()._lockWalkSpeed then
                hum.WalkSpeed = getgenv().WalkSpeedValue
            end
        end)
    end
end
Players.LocalPlayer.CharacterAdded:Connect(function(char) _applySpeedJump(char) end)
if Players.LocalPlayer.Character then _applySpeedJump(Players.LocalPlayer.Character) end

Tabs.Player:AddSlider("SliderSpeed", {
    ["Title"] = "Speed Chạy",
    ["Min"] = 16,
    ["Max"] = 300,
    ["Default"] = 30,
...
Tabs.Player:AddSlider("SliderJump", {
    ["Title"] = "Nhảy Cao",
    ["Min"] = 50,
    ["Max"] = 500,
    ["Default"] = 50,
...
-- ===== AUTO PLAYER HUNTER (PVP QUEST) =====
Tabs.Player:AddSection("Săn Người Chơi")
Tabs.Player:AddButton({
    ["Title"] = "Nhận Nhiệm Vụ Elite Player",
    ["Description"] = "Nhận Quest Săn Người Chơi",
    ["Callback"] = function()
        game.ReplicatedStorage.Remotes.CommF_:InvokeServer("PlayerHunter")
    end
})
Tabs.Player:AddToggle("ToggleAutoPlayerHunter", {
    ["Title"] = "Auto Kill Player Quest",
    ["Description"] = "Bay Đến Người Chơi Đã Nhận Nhiệm Vụ",
    ["Default"] = false
}):OnChanged(function(v)
...
spawn(function()
    while wait() do
        if _G.AutoPlayerHunter then
            pcall(function()
                local Quest = Players.LocalPlayer.PlayerGui.Main.Quest
                if not Quest.Visible then
                    wait(0.5)
                    game.ReplicatedStorage.Remotes.CommF_:InvokeServer("PlayerHunter")
                else
                    for _, char in pairs(workspace.Characters:GetChildren()) do
                        if string.find(Quest.Container.QuestTitle.Title.Text, char.Name) then
                            repeat
                                wait()
                                AutoHaki()
                                EquipTool(SelectWeapon)
                                Tween2(char.HumanoidRootPart.CFrame * CFrame.new(1, 7, 3))
                                char.HumanoidRootPart.Size = Vector3.new(60, 60, 60)
                                game:GetService("VirtualUser"):CaptureController()
                                game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                            until _G.AutoPlayerHunter == false or char.Humanoid.Health <= 0
...
-- ===== SAFE MODE - Bay Lên Trời =====
Tabs.Player:AddToggle("ToggleSafeMode", {
    ["Title"] = "Safe Mode (Bay Lên Trời An Toàn)",
    ["Description"] = "Tự Động Bay Lên Cao Tránh Bị Tấn Công",
    ["Default"] = false
}):OnChanged(function(v)
    _G.SafeMode = v
end)
        ...
        while wait() do
            if _G.SafeMode then
                Players.LocalPlayer.Character.HumanoidRootPart.CFrame =
                    Players.LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0, 200, 0)
            end
        end
    end)
end)
...
-- ===== BERRY ESP =====
Tabs.Fruit:AddToggle("ToggleBerryESP", {
    ["Title"] = "Định Vị Cây Berry",
    ["Description"] = "Hiện Cây Berry Trên Map",
    ["Default"] = false
}):OnChanged(function(v)
    Berry = v
    if not Berry then
        for _, part in pairs(game:GetService("CollectionService"):GetTagged("BerryBush")) do
            if part.Parent:FindFirstChild("BerryESP") then
                part.Parent.BerryESP:Destroy()
            end
        end
    else
        spawn(function()
            while Berry do
                pcall(function()
                    for _, bush in pairs(game:GetService("CollectionService"):GetTagged("BerryBush")) do
                        ...
                        if Berry then
                            if not parent:FindFirstChild("BerryESP") then
                            ...
                            if parent:FindFirstChild("BerryESP") then parent.BerryESP:Destroy() end
                        end
                    end
                end)
                task.wait(2)
...
-- ===== CHECK STOCK TRÁI TỪ KHO =====
Tabs.Fruit:AddButton({
    ["Title"] = "Xem Kho Trái (Advance Dealer)",
    ["Description"] = "Kiểm Tra Trái Trong Kho Advance Fruit Dealer",
    ["Callback"] = function()
        pcall(function()
            local result = game.ReplicatedStorage.Remotes.CommF_:InvokeServer("GetFruits", true)
            if result then
                local msg = "== Kho Trái ==\n"
                for _, item in ipairs(result) do
                    if type(item) == "table" and item.Name then
                        msg = msg .. item.Name .. ": " .. (item.Price or "?") .. " Robux\n"
                    end
                end
                Library:Notify({Title = "Kho Trái", Description = msg:sub(1, 200), Duration = 10})
            end
        end)
    end
})
...
-- ===== DELETE LAVA (Misc) =====
Tabs.Misc:AddSection("Xóa Lava")
Tabs.Misc:AddToggle("ToggleDeleteLava", {
    ["Title"] = "Xóa Lava (Tránh Bị Lava Làm Phiền)",
    ["Description"] = "Xóa Lava Tránh Bị Máy Thằng Kid Dìm Lava",
    ["Default"] = false
}):OnChanged(function(v)
    _G.RemoveLava = v
end)
...
-- ===== AUTO STORE FRUIT (cải tiến, đầy đủ hơn bản cũ) =====
Tabs.Fruit:AddSection("Lưu Trái Đầy Đủ")
Tabs.Fruit:AddToggle("ToggleAutoStoreFruitFull", {
    ["Title"] = "Auto Store Fruits (Đầy Đủ)",
    ["Description"] = "Tự Động Lưu Trái Ác Quỷ Vào Kho Đồ (Đầy Đủ Hơn)",
    ["Default"] = false
}):OnChanged(function(v)
    _G.AutoStoreFruitFull = v
end)
spawn(function()
    while task.wait(0.2) do
        if _G.AutoStoreFruitFull then
            pcall(function()
                ...
                for _, pair in ipairs(fruitMap) do
                    local name, key = pair[1], pair[2]
                    local fruit = bp:FindFirstChild(name) or char:FindFirstChild(name)
                    if fruit then
                        game.ReplicatedStorage.Remotes.CommF_:InvokeServer("StoreFruit", key, fruit)
                        break
                    end
                end
            end)
...
-- ===== AUTO DRAGON HUNTER (Draco) =====
Tabs.Sea:AddSection("Dragon Hunter")
Tabs.Sea:AddToggle("ToggleAutoDragonHunter", {
    ["Title"] = "Auto Dragon Hunter (Farm Blaze)",
    ["Description"] = "Tự Động Farm Dragon Dojo",
    ["Default"] = false
}):OnChanged(function(v)
    _G.FarmBlazeEM = v
end)
local function _checkDragonQuest()
    local info = {}
    pcall(function()
        game.ReplicatedStorage.Modules.Net["RF/DragonHunter"]:InvokeServer(unpack({{Context = "RequestQuest"}}))
    end)
    local ok, result = pcall(function()
        return game.ReplicatedStorage.Modules.Net["RF/DragonHunter"]:InvokeServer(unpack({{Context = "Check"}}))
    end)
    if ok and result and result.Text then
        info.HasQuest = true
        local txt = result.Text
        if string.find(txt, "Defeat") then
            info.Type = "Kill"
            info.Count = tonumber(string.sub(txt, 8, 9)) or 1
        end
    end
    return info
end
spawn(function()
    while task.wait(0.5) do
        if _G.FarmBlazeEM then
            pcall(function()
                local qInfo = _checkDragonQuest()
                if qInfo.HasQuest then
                    for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                        if (string.find(enemy.Name, "Hydra") or string.find(enemy.Name, "Venomous") or string.find(enemy.Name, "Draco"))
                        ...
                        then
                            repeat
                                ...
                                AttackNoCoolDown()
                            until not _G.FarmBlazeEM or not enemy.Parent or enemy.Humanoid.Health <= 0
                        end
                    end
                end
            end)
