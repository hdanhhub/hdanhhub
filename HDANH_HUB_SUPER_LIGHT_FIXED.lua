...
for i, url in ipairs(ui_urls) do
    ...
    if success and result then
        -- FORCE config trước khi tạo window
        local windowConfig = {
            Title = "HDANH HUB",
            SubTitle = "by Hdanh | NOPERMC",
            SaveFolder = "HDANH_Hub_Config",
        }
        
        -- Thử set size nếu library hỗ trợ
        pcall(function()
            windowConfig.Size = UDim2.fromOffset(700, 480)
        end)
        
        v466 = result:MakeWindow(windowConfig)
        ui_loaded = true
        print("✅ UI loaded from URL " .. i)
        
        -- Notification
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "🎨 HDANH HUB - SUPER LIGHT";
            Text = "Đang tải theme SIÊU NHẠT...\n✨ Vòng tròn sẽ SIÊU RÕ!\nVui lòng đợi 3 giây!";
            Duration = 4;
        })
        
        -- FORCE ĐỔI MÀU HỒNG NGAY SAU KHI TẠO WINDOW - AGGRESSIVE
        spawn(function()
            for delay = 1, 10 do  -- Chạy 10 lần trong 5 giây đầu
                wait(0.5)
                pcall(function()
                    -- Tìm window vừa tạo
                    for _, gui in pairs(game:GetService("CoreGui"):GetChildren()) do
                        if gui:IsA("ScreenGui") and (gui.Name:find("Redz") or gui.Name:find("Library") or gui.Name:find("UI")) then
                            for _, obj in pairs(gui:GetDescendants()) do
                                
                                -- AGGRESSIVE - Đổi MỌI nền SÁNG thành TỐI HỒNG
                                if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                                    local c = obj.BackgroundColor3
                                    local brightness = (c.R + c.G + c.B) / 3
                                    
                                    -- Nền sáng (brightness > 0.7) -> Nền tối hồng
                                    if brightness > 0.7 then
                                        obj.BackgroundColor3 = Color3.fromRGB(25, 20, 25)
                                        if delay == 1 then print("✅ Đổi nền sáng -> tối: " .. obj.Name) end
                                    end
                                    
                                    -- Nền vàng -> Hồng SIÊU NHẠT với GRADIENT
                                    if c.R > 0.7 and c.G > 0.6 and c.B < 0.5 then
                                        obj.BackgroundColor3 = Color3.fromRGB(255, 220, 240)  -- SIÊU NHẠT
                                        
                                        -- Thêm gradient SIÊU NHẠT
                                        if not obj:FindFirstChild("PinkGradient") then
                                            local gradient = Instance.new("UIGradient")
                                            gradient.Name = "PinkGradient"
                                            gradient.Color = ColorSequence.new{
                                                ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 230, 245)),  -- Pastel cực nhạt
                                                ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 210, 235))   -- Pastel nhạt
                                            }
                                            gradient.Rotation = 45
                                            gradient.Parent = obj
                                        end
                                        
                                        if delay == 1 then print("✅ Đổi vàng -> hồng SIÊU NHẠT: " .. obj.Name) end
                                    end
                                end
                                
                                -- AGGRESSIVE - Đổi NÚT SIÊU NHẠT
                                if obj:IsA("TextButton") then
                                    local c = obj.BackgroundColor3
                                    
                                    -- Nút vàng -> Hồng ON SIÊU NHẠT
                                    if c.R > 0.7 and c.G > 0.6 and c.B < 0.5 then
                                        obj.BackgroundColor3 = Color3.fromRGB(255, 220, 240)  -- SIÊU NHẠT
                                        
                                        -- Thêm gradient SIÊU NHẠT cho button ON
                                        if not obj:FindFirstChild("ButtonGradient") then
                                            local gradient = Instance.new("UIGradient")
                                    ...
                                    -- Nút xám/tối -> Hồng OFF SIÊU NHẠT
                                    local brightness = (c.R + c.G + c.B) / 3
                                    if brightness > 0.3 and brightness < 0.6 then
                                        obj.BackgroundColor3 = Color3.fromRGB(220, 200, 215)  -- SIÊU NHẠT
                                        
                                        -- Thêm gradient SIÊU NHẠT cho button OFF
                                        if not obj:FindFirstChild("ButtonGradientOff") then
                                            local gradient = Instance.new("UIGradient")
                                            gradient.Name = "ButtonGradientOff"
                                            gradient.Color = ColorSequence.new{
                                                ColorSequenceKeypoint.new(0, Color3.fromRGB(235, 215, 230)),  -- Xám hồng cực nhạt
                                                ColorSequenceKeypoint.new(1, Color3.fromRGB(215, 195, 210))   -- Xám hồng nhạt
                                            }
                                            gradient.Rotation = 45
                                            gradient.Parent = obj
                                        end
                                ...
                                if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                                    ...
                                    if tc.R > 0.7 and tc.G > 0.6 and tc.B < 0.5 then
                                        obj.TextColor3 = Color3.fromRGB(255, 182, 193)
                                    end
                                end
                            end
                            
                            -- PHÓNG TO MAIN FRAME
                            local mainFrame = gui:FindFirstChild("Main") or gui:FindFirstChild("MainFrame") or gui:FindFirstChild("Container")
                            if mainFrame and mainFrame:IsA("Frame") then
                                mainFrame.Size = UDim2.fromOffset(700, 480)
                                mainFrame.Position = UDim2.new(0.5, -350, 0.5, -240)
                                if delay == 1 then print("✅ Đã phóng to UI: 700x480") end
                            end
                        end
                    end
                end)
            ...
            wait(1)
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "✅ HDANH HUB - SUPER LIGHT THEME";
                Text = "Theme SIÊU NHẠT hoàn tất!\n✨ Background cực nhạt - Vòng tròn SIÊU RÕ\n🎚️ Toggle: Trượt trái/phải\n💎 Chúc farm vui!";
                Duration = 6;
            })
        end)
...
if not ui_loaded then
    ...
    -- Tạo notification
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "⚠️ HDANH HUB - Lỗi!";
        Text = "Không thể load UI!\nKRNL có thể không hỗ trợ HttpGet.\nThử executor khác!";
        Duration = 10;
    })
    
    -- Tạo UI đơn giản thay thế - PHÓNG TO VÀ MÀU HỒNG
    ...
    main.Name = "MainFrame"
    main.Size = UDim2.new(0, 600, 0, 400)  -- Phóng to từ 450x300 -> 600x400
    main.Position = UDim2.new(0.5, -300, 0.5, -200)
    main.BackgroundColor3 = Color3.fromRGB(30, 30, 35)  -- Nền tối
    ...
    title.Size = UDim2.new(1, 0, 0, 55)  -- Tăng từ 45 -> 55
    title.BackgroundColor3 = Color3.fromRGB(255, 210, 235)  -- Hồng SIÊU NHẠT
    title.Text = "💎 HDANH HUB - Blox Fruits Premium"
    title.TextColor3 = Color3.fromRGB(100, 100, 100)  -- Chữ xám để nổi bật
    title.Font = Enum.Font.GothamBold
    title.TextSize = 20  -- Tăng từ 18 -> 20
    title.Parent = main
    ...
    -- THÊM GRADIENT SIÊU NHẠT cho title
    local titleGradient = Instance.new("UIGradient")
    titleGradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 230, 245)),  -- Pastel cực nhạt
        ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 200, 230))   -- Pastel nhạt
    }
    titleGradient.Rotation = 90  -- Từ trên xuống
    titleGradient.Parent = title
    
    local closeBtn = Instance.new("TextButton")
    closeBtn.Size = UDim2.new(0, 45, 0, 45)  -- Tăng từ 35x35 -> 45x45
    closeBtn.Position = UDim2.new(1, -50, 0, 5)
    closeBtn.Text = "✖"
    closeBtn.BackgroundColor3 = Color3.fromRGB(255, 150, 180)  -- Hồng vừa - vẫn thấy được
    closeBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
    closeBtn.Font = Enum.Font.GothamBold
    closeBtn.TextSize = 24  -- Tăng từ 20 -> 24
    ...
    -- THÊM GRADIENT nhẹ cho close button
    local closeBtnGradient = Instance.new("UIGradient")
    closeBtnGradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 180, 200)),  -- Hồng nhạt
        ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 140, 180))   -- Hồng vừa
    }
    closeBtnGradient.Rotation = 135  -- Góc chéo
    ...
    infoText.Size = UDim2.new(1, -40, 1, -75)  -- Điều chỉnh cho size mới
...
✅ Giải pháp:
  ...
  4. Liên hệ HDANH HUB để hỗ trợ

⚠️ Script KHÔNG THỂ hoạt động với KRNL!
Vui lòng dùng executor khác!
]]
    infoText.TextColor3 = Color3.fromRGB(255, 255, 255)
    infoText.Font = Enum.Font.Gotham
    infoText.TextSize = 15  -- Tăng từ 13 -> 15
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "✅ HDANH HUB";
    Text = "UI đã load thành công!\n💎 Đang áp dụng theme SIÊU NHẠT...";
    Duration = 5;
})

-- FORCE MÀU HỒNG + FIX BACKGROUND TRẮNG VÀNG - Override cực mạnh
spawn(function()
    while wait(0.5) do  -- Check mỗi 0.5s cho nhanh hơn
        pcall(function()
            for _, gui in pairs(game:GetService("CoreGui"):GetChildren()) do
                if gui:IsA("ScreenGui") and gui.Name ~= "ControlGUI" and gui.Name ~= "HDANHBackupUI" then
                    for _, obj in pairs(gui:GetDescendants()) do
                        
                        -- FIX BACKGROUND TRẮNG/VÀNG
                        if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                            local c = obj.BackgroundColor3
                            
                            -- Phát hiện màu TRẮNG (R>0.9, G>0.9, B>0.9)
                            if c.R > 0.9 and c.G > 0.9 and c.B > 0.9 then
                                obj.BackgroundColor3 = Color3.fromRGB(25, 20, 25)  -- NỀN TỐI HỒNG
                            end
                            
                            -- Phát hiện màu VÀNG -> SIÊU NHẠT với GRADIENT
                            if c.R > 0.7 and c.G > 0.6 and c.B < 0.5 then
                                obj.BackgroundColor3 = Color3.fromRGB(255, 220, 240)  -- SIÊU NHẠT
                                
                                -- Thêm gradient SIÊU NHẠT nếu chưa có
                                if not obj:FindFirstChild("PinkGradient") then
                                    local gradient = Instance.new("UIGradient")
                                    gradient.Name = "PinkGradient"
                                    gradient.Color = ColorSequence.new{
                            ...
                            -- Phát hiện màu XÁM NHẠT (background secondary)
                            if c.R > 0.7 and c.R < 0.85 and c.G > 0.7 and c.G < 0.85 and c.B > 0.7 and c.B < 0.85 then
                                obj.BackgroundColor3 = Color3.fromRGB(35, 25, 35)  -- NỀN TỐI HỒNG NHẠT
                            end
                            
                            -- Nền xám đậm -> tối hồng
                            if c.R < 0.2 and c.G < 0.2 and c.B < 0.2 then
                                obj.BackgroundColor3 = Color3.fromRGB(20, 15, 20)  -- RẤT TỐI HỒNG
                            end
                        end
                        
                        -- FIX TOGGLE BUTTON - NHƯNG BỎ QUA iOS TOGGLES
                        if obj:IsA("TextButton") and not obj.Name:find("iOSToggle") and obj.Visible then
                            local c = obj.BackgroundColor3
                            
                            -- Nút màu vàng -> Ẩn đi (sẽ được thay bằng iOS toggle)
                            if c.R > 0.7 and c.G > 0.6 and c.B < 0.5 then
                                -- Đánh dấu để tạo iOS toggle
                                obj.BackgroundTransparency = 1  -- Ẩn button cũ
                                obj.TextTransparency = 1
                            end
                        end
                        
                        -- FIX TEXT MÀU VÀNG -> HỒNG
                        if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                            local tc = obj.TextColor3
                            
                            -- Text vàng -> hồng
                            if tc.R > 0.7 and tc.G > 0.6 and tc.B < 0.5 then
                                obj.TextColor3 = Color3.fromRGB(255, 182, 193)  -- LIGHT PINK
                            end
                            
                            -- Text xám nhạt -> trắng sáng
                            if tc.R > 0.6 and tc.R < 0.8 and tc.G > 0.6 and tc.G < 0.8 and tc.B > 0.6 and tc.B < 0.8 then
                                obj.TextColor3 = Color3.fromRGB(255, 255, 255)  -- TRẮNG
                            end
                        end
                        ...
                        if obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
                            ...
                            if ic.R > 0.7 and ic.G > 0.6 and ic.B < 0.5 then
                                obj.ImageColor3 = Color3.fromRGB(255, 105, 180)
                            end
                        end
                        ...
                        if obj:IsA("UIStroke") then
                            local sc = obj.Color
                            if sc.R > 0.7 and sc.G > 0.6 and sc.B < 0.5 then
                                obj.Color = Color3.fromRGB(255, 105, 180)
                            end
                            -- Viền trắng -> hồng nhạt
                            if sc.R > 0.9 and sc.G > 0.9 and sc.B > 0.9 then
                                obj.Color = Color3.fromRGB(255, 182, 193)
                            end
                        end
                        
                        -- FIX UIGRADIENT nếu có
...
-- PHÓNG TO UI WINDOW - Check liên tục
spawn(function()
    while wait(1) do
        pcall(function()
            for _, gui in pairs(game:GetService("CoreGui"):GetChildren()) do
                if gui:IsA("ScreenGui") and gui.Name ~= "ControlGUI" and gui.Name ~= "HDANHBackupUI" then
                    ...
                    for _, frame in pairs(gui:GetDescendants()) do
                        if frame:IsA("Frame") and (frame.Name == "Main" or frame.Name == "MainFrame" or frame.Name == "Container") then
                            if frame.Size.X.Offset < 600 or frame.Size.Y.Offset < 400 then
                                frame.Size = UDim2.fromOffset(700, 480)  -- TĂNG LÊN 700x480 cho rộng hơn
                        ...
                        if frame:IsA("TextLabel") or frame:IsA("TextButton") then
                            if frame.TextSize > 0 and frame.TextSize < 16 then
                                frame.TextSize = frame.TextSize + 2
                            end
                        end
                    end
                end
...
-- CUSTOM iOS TOGGLE SWITCH - Slider trượt trái/phải
spawn(function()
    wait(2)  -- Đợi UI load xong
    pcall(function()
        local TweenService = game:GetService("TweenService")
        local processedToggles = {}
        
        -- Function tạo iOS toggle switch
        local function CreateiOSToggle(originalButton)
            if processedToggles[originalButton] then return end
            processedToggles[originalButton] = true
            
            -- Lấy thông tin ban đầu
            local parent = originalButton.Parent
            local position = originalButton.Position
            local size = originalButton.Size
            local text = originalButton.Text
            ...
            toggleContainer.Name = "iOSToggle_" .. originalButton.Name
            toggleContainer.Size = UDim2.new(0, 60, 0, 30)  -- Toggle nhỏ gọn
            toggleContainer.Position = UDim2.new(1, -70, 0.5, -15)  -- Bên phải
            toggleContainer.AnchorPoint = Vector2.new(0, 0.5)
            toggleContainer.BackgroundColor3 = Color3.fromRGB(220, 200, 215)  -- OFF: RẤT NHẠT
            toggleContainer.BorderSizePixel = 0
            toggleContainer.Parent = parent
            
            local toggleCorner = Instance.new("UICorner")
            toggleCorner.CornerRadius = UDim.new(0.5, 0)  -- Bo tròn hoàn toàn
            toggleCorner.Parent = toggleContainer
            
            -- Thêm GRADIENT cho toggle container - SIÊU NHẠT
            local toggleGradient = Instance.new("UIGradient")
            toggleGradient.Name = "ToggleGradient"
            -- Bắt đầu với gradient OFF (hồng SIÊU SIÊU NHẠT)
            toggleGradient.Color = ColorSequence.new{
                ColorSequenceKeypoint.new(0, Color3.fromRGB(235, 215, 230)),  -- Hồng cực nhạt
                ColorSequenceKeypoint.new(1, Color3.fromRGB(215, 195, 210))   -- Hồng rất nhạt
            }
            toggleGradient.Rotation = 45  -- Góc xiên đẹp
            toggleGradient.Parent = toggleContainer
            
            -- Tạo slider (nút tròn) - TO VÀ NỔI BẬT
            local slider = Instance.new("Frame")
            slider.Name = "Slider"
            slider.Size = UDim2.new(0, 26, 0, 26)  -- To hơn
            slider.Position = UDim2.new(0, 2, 0.5, 0)
            slider.AnchorPoint = Vector2.new(0, 0.5)
            slider.BackgroundColor3 = Color3.fromRGB(255, 255, 255)  -- Slider trắng tinh
            slider.BorderSizePixel = 0
            slider.Parent = toggleContainer
            slider.ZIndex = 2  -- Đảm bảo slider luôn ở trên
            
            -- Thêm bóng ĐẬM cho slider để SIÊU nổi bật
            local sliderShadow = Instance.new("UIStroke")
            sliderShadow.Color = Color3.fromRGB(150, 150, 150)  -- Xám đậm hơn
            sliderShadow.Thickness = 2  -- Dày hơn
            sliderShadow.Transparency = 0.3  -- Đậm hơn
            sliderShadow.Parent = slider
            
            local sliderCorner = Instance.new("UICorner")
            sliderCorner.CornerRadius = UDim.new(1, 0)  -- Tròn hoàn toàn
            sliderCorner.Parent = slider
            
            -- State tracking
            local isEnabled = false
            
            -- Function cập nhật toggle - FIX: Màu chỉ thay đổi KHI CLICK, không tự động đổi
            local function UpdateToggle(enabled, animate)
                isEnabled = enabled
                
                if animate then
                    -- Animation trượt
                    local targetPos = enabled and UDim2.new(1, -28, 0.5, 0) or UDim2.new(0, 2, 0.5, 0)
                    
                    local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                    local tweenSlider = TweenService:Create(slider, tweenInfo, {Position = targetPos})
                    
                    -- QUAN TRỌNG: Chỉ đổi màu SAU KHI animation hoàn tất
                    tweenSlider.Completed:Connect(function()
                        if enabled then
                            -- ON: Gradient hồng nhạt pastel
                            toggleGradient.Color = ColorSequence.new{
                                ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 220, 240)),  -- Hồng pastel cực nhạt
                                ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 200, 230))   -- Hồng pastel nhạt
                            }
                        else
                            -- OFF: Gradient xám hồng cực nhạt
                            toggleGradient.Color = ColorSequence.new{
                                ColorSequenceKeypoint.new(0, Color3.fromRGB(235, 215, 230)),  -- Xám hồng cực nhạt
                                ColorSequenceKeypoint.new(1, Color3.fromRGB(215, 195, 210))   -- Xám hồng nhạt
                            }
                    ...
                    -- Không animation - set màu NGAY
                    slider.Position = enabled and UDim2.new(1, -28, 0.5, 0) or UDim2.new(0, 2, 0.5, 0)
                    ...
                    if enabled then
                        ...
                        toggleGradient.Color = ColorSequence.new{
                            ColorSequenceKeypoint.new(0, Color3.fromRGB(235, 215, 230)),
                            ColorSequenceKeypoint.new(1, Color3.fromRGB(215, 195, 210))
                        }
                    end
                end
            end
            ...
            toggleContainer.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    originalButton.MouseButton1Click:Fire()
                end
            end)
            ...
            -- Set initial state (giả sử OFF)
            UpdateToggle(false, false)
            
            print("✅ Tạo iOS toggle SIÊU NHẠT cho: " .. originalButton.Name)
        end
        
        -- Tìm và convert tất cả toggle buttons
        for _, gui in pairs(game:GetService("CoreGui"):GetChildren()) do
            if gui:IsA("ScreenGui") and gui.Name ~= "ControlGUI" and gui.Name ~= "HDANHBackupUI" then
                for _, obj in pairs(gui:GetDescendants()) do
                    -- Tìm các toggle buttons
                    if obj:IsA("TextButton") then
                        local objName = obj.Name:lower()
                        local objText = obj.Text:lower()
                        
                        -- Phát hiện toggle button
                        if objName:find("toggle") or objText:find("auto") or objText:find("enable") or 
                           objName:find("switch") or obj.Parent.Name:find("Toggle") then
                            CreateiOSToggle(obj)
                        end
                    end
                end
        ...
        -- Monitor cho toggle mới được tạo
        game:GetService("CoreGui").DescendantAdded:Connect(function(obj)
            wait(0.1)
            if obj:IsA("TextButton") then
                local objName = obj.Name:lower()
                if objName:find("toggle") or objName:find("switch") then
                    CreateiOSToggle(obj)
                end
            end
        end)
...
-- Toggle Button (Icon mở/đóng menu) - PHÓNG TO VÀ MÀU HỒNG
local l_ScreenGui_0 = Instance.new("ScreenGui")
l_ScreenGui_0.Name = "ControlGUI"
l_ScreenGui_0.Parent = game.CoreGui
local l_ImageButton_0 = Instance.new("ImageButton")
l_ImageButton_0.Size = UDim2.new(0, 65, 0, 65)  -- Phóng to từ 50x50 -> 65x65
l_ImageButton_0.Position = UDim2.new(0.15, 0, 0.15, 0)
l_ImageButton_0.Image = "rbxassetid://100472177647993"
...
l_UIStroke_0.Thickness = 3  -- Tăng độ dày viền từ 2 -> 3
l_UIStroke_0.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
l_UIStroke_0.Parent = l_ImageButton_0

-- THEME MÀU HỒNG - Gradient hồng thay vì rainbow
local v471 = {
    ...
    Color3.fromRGB(255, 105, 180),  -- Hot Pink (lặp lại)
}
task.spawn(function()
    local v472 = 1
    while true do
        ...
        v472 = v472 % #v471 + 1
        task.wait(0.4)  -- Chậm hơn một chút
    end
end)
local v473 = true
local v474 = nil
...
local v484 = v466:MakeTab({"Th\195\180ng Tin | Info","rbxassetid://100472177647993"})
local v485 = v466:MakeTab({"Farming","rbxassetid://100472177647993"})
local v486 = v466:MakeTab({"Auto Fishing","rbxassetid://100472177647993"})
local v487 = v466:MakeTab({"Quest | Items","rbxassetid://100472177647993"})
local v488 = v466:MakeTab({"Volcano Dojo","rbxassetid://100472177647993"})
local v489 = v466:MakeTab({"Sea Event","rbxassetid://100472177647993"})
local v490 = v466:MakeTab({"Race V4","rbxassetid://100472177647993"})
local v491 = v466:MakeTab({"Raid Fruits","rbxassetid://100472177647993"})
local v492 = v466:MakeTab({"Fruits | Check Stock","rbxassetid://100472177647993"})
local v493 = v466:MakeTab({"Teleport","rbxassetid://100472177647993"})
local v494 = v466:MakeTab({"PvP,Player","rbxassetid://100472177647993"})
local v495 = v466:MakeTab({"Shop","rbxassetid://100472177647993"})
local v496 = v466:MakeTab({"Settings","rbxassetid://100472177647993"})
