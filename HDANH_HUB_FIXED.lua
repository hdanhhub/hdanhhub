...
for i, url in ipairs(ui_urls) do
    ...
    if success and result then
        ...
        spawn(function()
            ...
            wait(1)
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "✅ HDANH HUB - PINK THEME";
                Text = "Theme hồng đã áp dụng!\n🎚️ Toggle mới: Trượt trái/phải\n💎 Chúc farm vui!";
                Duration = 5;
            })
        end)
...
-- CUSTOM SQUARE TOGGLE SWITCH - Hình vuông bo tròn màu hồng
spawn(function()
    ...
    pcall(function()
        ...
        local function CreateiOSToggle(originalButton)
            ...
            -- Tạo container cho toggle - HÌNH VUÔNG BO TRÒN
            local toggleContainer = Instance.new("Frame")
            toggleContainer.Name = "iOSToggle_" .. originalButton.Name
            toggleContainer.Size = UDim2.new(0, 40, 0, 40)  -- Vuông 40x40
            toggleContainer.Position = UDim2.new(1, -50, 0.5, -20)  -- Bên phải
            toggleContainer.AnchorPoint = Vector2.new(0, 0.5)
            toggleContainer.BackgroundColor3 = Color3.fromRGB(255, 105, 180)  -- Viền hồng
            toggleContainer.BorderSizePixel = 0
            toggleContainer.Parent = parent
            
            -- Bo tròn góc cho hình vuông
            local toggleCorner = Instance.new("UICorner")
            toggleCorner.CornerRadius = UDim.new(0, 8)  -- Bo tròn vừa phải
            toggleCorner.Parent = toggleContainer
            
            -- Tạo viền bên trong (inner square)
            local innerSquare = Instance.new("Frame")
            innerSquare.Name = "InnerSquare"
            innerSquare.Size = UDim2.new(0, 34, 0, 34)  -- Nhỏ hơn outer 3px mỗi bên
            innerSquare.Position = UDim2.new(0.5, 0, 0.5, 0)
            innerSquare.AnchorPoint = Vector2.new(0.5, 0.5)
            innerSquare.BackgroundColor3 = Color3.fromRGB(30, 30, 35)  -- Nền tối (OFF)
            innerSquare.BorderSizePixel = 0
            innerSquare.Parent = toggleContainer
            
            local innerCorner = Instance.new("UICorner")
            innerCorner.CornerRadius = UDim.new(0, 6)  -- Bo tròn tương ứng
            innerCorner.Parent = innerSquare
            
            -- Tạo nút check bên trong (hiện khi ON)
            local checkMark = Instance.new("Frame")
            checkMark.Name = "CheckMark"
            checkMark.Size = UDim2.new(0, 20, 0, 20)
            checkMark.Position = UDim2.new(0.5, 0, 0.5, 0)
            checkMark.AnchorPoint = Vector2.new(0.5, 0.5)
            checkMark.BackgroundColor3 = Color3.fromRGB(255, 105, 180)  -- Hồng (ON)
            checkMark.BorderSizePixel = 0
            checkMark.BackgroundTransparency = 1  -- Ẩn ban đầu (OFF)
            ...
            local function UpdateToggle(enabled, animate)
                ...
                if animate then
                    ...
                    local tweenCheck = TweenService:Create(checkMark, tweenInfo, {BackgroundTransparency = targetTransparency})
                    tweenCheck:Play()
                else
                    -- Không animation
                    checkMark.BackgroundTransparency = enabled and 0 or 1
                end
            end
            ...
            toggleContainer.MouseLeave:Connect(function()
                local stroke = toggleContainer:FindFirstChild("HoverStroke")
