...
for i, url in ipairs(ui_urls) do
    ...
    if success and result then
                            ...
                            for _, obj in pairs(gui:GetDescendants()) do
                                ...
                                if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                                    ...
                                    if brightness > 0.7 then
                                        ...
                                        if delay == 1 then print("" .. obj.Name) end
                                    end
        ...
        warn(" Failed to load from URL " .. i .. ": " .. tostring(result))
    end
end
...
if not ui_loaded then
    ...
    infoText.Size = UDim2.new(1, -40, 1, -75)
    infoText.Position = UDim2.new(0, 20, 0, 60)
    infoText.BackgroundTransparency = 1
    infoText.Text = [[
 LI: Khng th load UI Library!

 Nguyn nhn c th:
   KRNL khng h tr game:HttpGet()
   URL b chn hoc khng truy cp c
   Khng c kt ni internet
   GitHub/Pastefy b chn

 Gii php:
  1. Dng Executor tt hn:
      Solara (FREE, tt nht)
      Fluxus (FREE)
     - Wave (FREE)
     - Delta (FREE)
  ...
  3. Kim tra kt ni internet
  
  4. Lin h HDANH HUB  h tr

 Script KHNG TH hot ng vi KRNL!
Vui lng dng executor khc!
]]
                    ...
                    for _, obj in pairs(gui:GetDescendants()) do
                        ...
                        if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                            ...
                            if c.R > 0.7 and c.R < 0.85 and c.G > 0.7 and c.G < 0.85 and c.B > 0.7 and c.B < 0.85 then
                                obj.BackgroundColor3 = Color3.fromRGB(35, 25, 35)
                            end
                            
                            if c.R < 0.2 and c.G < 0.2 and c.B < 0.2 then
                                obj.BackgroundColor3 = Color3.fromRGB(20, 15, 20)
                            end
                        end
...
spawn(function()
    ...
    pcall(function()
        ...
        local function CreateiOSToggle(originalButton)
            ...
            toggleContainer.Position = UDim2.new(1, -70, 0.5, -15)
            toggleContainer.AnchorPoint = Vector2.new(0, 0.5)
            toggleContainer.BackgroundColor3 = Color3.fromRGB(70, 35, 60)
            ...
            local function UpdateToggle(enabled, animate)
                ...
                if animate then
                    ...
                    local targetColor = enabled and Color3.fromRGB(255, 105, 180) or Color3.fromRGB(70, 35, 60)
                    
                    local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                    local tweenSlider = TweenService:Create(slider, tweenInfo, {Position = targetPos})
                    local tweenBg = TweenService:Create(toggleContainer, tweenInfo, {BackgroundColor3 = targetColor})
                    
                    tweenSlider:Play()
                    tweenBg:Play()
                else
                    slider.Position = enabled and UDim2.new(1, -27, 0.5, 0) or UDim2.new(0, 3, 0.5, 0)
                    toggleContainer.BackgroundColor3 = enabled and Color3.fromRGB(255, 105, 180) or Color3.fromRGB(70, 35, 60)
                end
            end
...
l_ImageButton_0.Size = UDim2.new(0, 65, 0, 65)
