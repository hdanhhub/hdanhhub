-- ============================================
-- HDANH HUB - Tương thích PC & Mobile Xeno
-- Toggle: OFF=Hồng, ON=Xanh
-- ============================================

-- Anti-crash (optional hooks)
pcall(function()
...
if game.PlaceId == 2753915549 then
    World1 = true
    print("🌊 HDANH HUB - Sea 1")
elseif game.PlaceId == 4442272183 then
    World2 = true
    print("🌊 HDANH HUB - Sea 2")
elseif game.PlaceId == 7449423635 then
    World3 = true
    print("🌊 HDANH HUB - Sea 3")
else
...
-- ============================================
-- LOAD UI - TƯƠNG THÍCH XENO PC & MOBILE
-- ============================================
...
for i, url in ipairs(ui_urls) do
    ...
    if success and result then
        v466 = result:MakeWindow({
            ...
            SaveFolder = "HDANH_Hub"
        })
        ui_loaded = true
        print("✅ UI loaded from URL " .. i)
        break
    else
        warn("❌ Failed URL " .. i .. ": " .. tostring(result))
    end
end

if not ui_loaded then
    error("❌ Cannot load UI library!")
end

game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "✅ HDANH HUB";
    Text = "UI loaded! Đang setup toggle...";
    Duration = 4;
})

-- ============================================
-- FORCE TOGGLE COLOR - SIMPLE APPROACH
-- ============================================

-- Table lưu state toggles
_G.ToggleColors = {}

-- Function force màu toggle
local function ForceToggleColors()
    pcall(function()
        for _, gui in pairs(game:GetService("CoreGui"):GetChildren()) do
            if gui:IsA("ScreenGui") and gui.Name ~= "ControlGUI" then
                for _, obj in pairs(gui:GetDescendants()) do
                    -- Tìm toggle button/frame
                    if obj:IsA("TextButton") or (obj:IsA("Frame") and obj.Name:find("Toggle")) then
                        local parent = obj.Parent or obj
                        
                        -- Tìm background frame
                        for _, child in pairs(parent:GetDescendants()) do
                            if child:IsA("Frame") and child.Name:find("Background") then
                                -- Kiểm tra state từ _G variables
                                local toggleName = obj.Text or obj.Name
                                local isEnabled = false
                                
                                -- Check common _G variables
                                if toggleName:find("Auto") or toggleName:find("AUTO") then
                                    local varName = "_G." .. toggleName:gsub(" ", "")
                                ...
                                -- Đổi màu dựa trên state
                                if isEnabled then
                                    child.BackgroundColor3 = Color3.fromRGB(74, 222, 128)  -- XANH (ON)
                                else
                                    child.BackgroundColor3 = Color3.fromRGB(255, 105, 180)  -- HỒNG (OFF)
                                end
                                
                                -- Tìm hoặc tạo circle
                                local circle = child.Parent:FindFirstChild("Circle")
                                if not circle then
                                ...
                                if circle then
                                    if isEnabled then
                                        circle.Position = UDim2.new(1, -25, 0.5, -11)  -- PHẢI
                                    else
                                        circle.Position = UDim2.new(0, 3, 0.5, -11)  -- TRÁI
                                    end
                                end
                            end
                        end
